...
waveforms_folder = './data/earth/waveforms'
catalogues_folder = './data/earth/catalogues'
...
def load_waveforms_and_labels(waveforms_folder, catalogues_folder, norm_percentile, labels_type='binary', channels=["_N.csv", "_E.csv", "_Z.csv"]): #, channels=["_N.csv", "_E.csv", "_Z.csv"], channels=["_Z.csv"]
    waveforms = []
    labels = []
...
def plot_picking_predictions(model, test_loader, device, num_samples=10):
    ...
    for i, (inputs, targets, outputs) in enumerate(samples[:num_samples]):
        ...
        # Plot the actual and predicted labels
        axes[i].plot(targets[0, :].numpy(), label='Actual', linestyle='--')
        axes[i].plot(outputs[0, :].numpy(), label='Predicted', linestyle=':')
...
def process_data(labels_type, percentile=0.998,plot=False, ):
    #Load the dataset
    waveforms, labels = load_waveforms_and_labels(waveforms_folder=waveforms_folder, catalogues_folder=catalogues_folder, norm_percentile=percentile, labels_type=labels_type)
